...
def showInstituteupdate():
    ...
    t.geometry('700x700')
    t.title('Institute')
    r=Label(t,text='Institute Update Form',font=('arial',20),fg='red',bg='skyblue')
    r.place(x=120,y=10)
    t.config(bg='teal')
    ...
    def updatedata():
        ...
        xe=e5.get()
        xf=e6.get()
        sql="update Institute set name='%s',address='%s',phone='%s',email='%s',regno='%s' where instid=%d"%(xb,xc,xd,xe,xf,xa)
        cur.execute(sql)
        db.commit()
        db.close()
    ...
    b1.place(x=50,y=300)
    b2=Button(t,text='update',bg='blue',command=updatedata,width=20)
