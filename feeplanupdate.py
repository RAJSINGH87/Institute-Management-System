...
import pymysql
def showfeeplanupdate():
    t=tkinter.Tk()
    t.geometry ('700x700')
    t.title('institute')
    r=Label(t,text='Feeplan Update Form',font=('arial',20),fg='red',bg='white')
    r.place(x=120,y=10)
    t.config(bg='navy')
    def finddata():
    ...
    def updatedata():
        ...
        sql="update feeplan set totalfee='%s',numberofinstallments='%s' where feeplanid=%d"%(xb,xc,xa)
        cur.execute(sql)
        db.commit()
        db.close()
    ...
    b1.place(x=50,y=250)
    b1=Button(t,text='update',bg='red',command=updatedata,width=20)
